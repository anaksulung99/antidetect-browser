...
const osOptions = computed(
  () => Object.keys(capabilities.value[deviceType.value] ?? {}) as OsName[]
);
...
const browserOptions = computed(() =>
  Object.keys(currentCapability.value?.browsers ?? {})
);
...
const filteredFingerprints = computed(() => {
  ...
  return fingerprints.value.filter((item) =>
    [item.name, item.osName, item.browserName, item.deviceType].some((value) =>
...
function syncDeviceDefaults() {
  ...
  if (!availableOs.includes(osName.value))
    osName.value = availableOs[0] ?? "windows";
  syncOsDefaults();
}
...
function syncOsDefaults() {
  ...
  if (!browsers.includes(browserName.value))
    browserName.value = browsers[0] ?? "";
...
async function loadData() {
  ...
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : "Unable to load fingerprints.";
  } finally {
    loading.value = false;
  }
}
...
async function showDetail(id: string) {
  clearMessages();
  try {
    selected.value = (await window.appRuntime.fingerprints.get({
      id,
    ...
    error.value =
      cause instanceof Error
        ? cause.message
        : "Unable to load fingerprint detail.";
  }
}
...
async function toggleStatus(item: FingerprintRecord) {
  ...
  try {
    ...
    await window.appRuntime.fingerprints.setStatus({
      id: item.id,
      status: nextStatus,
    });
...
async function removeFingerprint(item: FingerprintRecord) {
  ...
  try {
    ...
    error.value =
      cause instanceof Error ? cause.message : "Unable to delete fingerprint.";
  }
}
  ...
  <div class="space-y-6">
    ...
    >
      <div>
        ...
        <p class="text-muted-foreground">
          Generate and review consistent device fingerprints.
        </p>
      </div>
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription
          >Device selection automatically narrows compatible OS and browser
          options.</CardDescription
        >
      </CardHeader>
      <CardContent>
        <form
        ...
        >
          ...
          <div class="md:col-span-2">
            <Button type="submit" :disabled="saving || !currentCapability">{{
              saving ? "Generating..." : "Generate fingerprint"
            }}</Button>
          </div>
        </form>
      </CardContent>
    ...
    <Card>
      <CardHeader
        ...
        >
          <div>
            <CardTitle>Fingerprint list</CardTitle
            ><CardDescription
              >Preset version dan compatibility warning disimpan bersama
      ...
      <CardContent>
        <div class="overflow-x-auto">
          <Table
            ><TableHeader
              ><TableRow
                ><TableHead>Name</TableHead><TableHead>Device / OS</TableHead
                ><TableHead>Browser</TableHead><TableHead>Locale</TableHead
            ...
            ><TableBody>
              <TableRow v-if="loading"
                ...
                ></TableRow
              >
              <TableRow v-else-if="filteredFingerprints.length === 0"
                ><TableCell
                  colspan="6"
                  class="text-muted-foreground text-center"
                  >No fingerprints found.</TableCell
                ></TableRow
              >
              <TableRow
                ...
                ><TableCell class="capitalize"
                  >{{ item.deviceType }} / {{ item.osName }}
                  {{ item.osVersion }}</TableCell
                ><TableCell
                  >{{ item.browserName }} {{ item.browserVersion }}</TableCell
    ...
    <Card v-if="selected"
      ><CardHeader
        ><CardTitle>Fingerprint detail</CardTitle
        ><CardDescription
          >{{ selected.name }} · {{ selected.presetVersion }}</CardDescription
        ></CardHeader
      ><CardContent class="space-y-3"
        ><div v-if="selected.compatibilityWarnings?.length" class="space-y-2">
          <Badge variant="destructive">Compatibility warning</Badge>
          <ul class="text-muted-foreground list-disc pl-5 text-sm">
            <li
              v-for="warning in selected.compatibilityWarnings"
              :key="warning"
            >
              {{ warning }}
            </li>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          <div>
            <p class="text-muted-foreground text-xs">User agent</p>
            <p class="break-all text-sm">{{ selected.userAgent }}</p>
          </div>
          <div>
            <p class="text-muted-foreground text-xs">Timezone</p>
            <p class="text-sm">{{ selected.timezone }}</p>
          </div>
        </div>
